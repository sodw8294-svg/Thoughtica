...
/* ═══════════════════════════════════════════════════════════════
   AI SERVICE — replaces all static/scripted responses
   Every companion response is now generated by the LLM backend.
   ═══════════════════════════════════════════════════════════════ */

/**
 * Generic failure message that mirrors the one in src/index.html so both
 * UI layers present consistent copy when the backend is unreachable.
 */
function buildCompanionFailureMessage(err?: { error?: { code?: string; retryable?: boolean } } | null): string {
  const code = err?.error?.code ?? 'PROVIDER_UNAVAILABLE'
  if (code === 'PROVIDER_NOT_CONFIGURED') {
    return 'My connection to the AI is not set up yet. Your conversation context is still safe — please ask the sanctuary admin to configure an AI provider key.'
  }
  const retryable = err?.error?.retryable !== false
  return retryable
    ? 'I\'m having a little trouble reaching my thoughts right now. Your conversation context is still safe — please try again in a moment.'
    : 'Something unexpected happened. Your conversation context is still safe. Please refresh and try again.'
}

/**
 * Call /api/chat and return the assistant reply string.
 * On network / provider failure, returns a graceful failure message.
 *
 * @param payload  Fields matching the /api/chat request body contract
...
async function callChatAPI(
  ...
  try {
    ...
    if (!res.ok || !data?.reply) {
      return buildCompanionFailureMessage(data)
    }
    return data.reply as string
  } catch (err: unknown) {
    // AbortError means the caller cancelled — surface a neutral message
    if (err instanceof Error && err.name === 'AbortError') {
      return ''
    }
    return buildCompanionFailureMessage(null)
  }
}
...
function CompanionTab({ state, setState }: { state: AppState; setState: (s: AppState) => void }) {
  ...
  // AbortController ref so the user can cancel an in-flight request
  ...
  const sendMessage = async (text: string) => {
    ...
    const baseUpdated = { ...state, chatMessages: [...state.chatMessages, userMsg] }
    if (state.tier === 'free' && state.aiInteractionsRemaining > 0) {
      baseUpdated.aiInteractionsRemaining = state.aiInteractionsRemaining - 1
    }

    setState(baseUpdated)
    setInput('')
    ...
    // Build conversation history for context (last 16 exchanges)
    const historyForAPI = state.chatMessages
      .slice(-16)
    ...
    const reply = await callChatAPI(
      {
        ...
        userName: 'Seeker',
        userGoal: state.intention || 'personal growth and mindful living',
    ...
    // Silently discard cancelled/empty replies
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              {isLoading ? 'Thinking...' : PERSONAS[state.companion.persona].label}
            </p>
          </div>
          {state.tier === 'free' && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {/* Animated typing indicator while waiting for the LLM response */}
        {isLoading && (
      ...
      <div className="shrink-0 p-4 border-t border-border/60">
        <div className="flex items-center gap-2">
          ...
          />
          {/* Stop button — only shown while a request is in flight */}
          {isLoading ? (
            <button
              onClick={() => abortRef.current?.abort()}
              className="p-2.5 bg-muted text-muted-foreground rounded-xl hover:bg-destructive/20 hover:text-destructive transition-colors"
...
function JournalTab({ state, setState }: { state: AppState; setState: (s: AppState) => void }) {
  ...
  const requestAiReflection = async (entryId: string) => {
    ...
    if (!entry) { setAiLoading(false); return }

    // Ask the LLM for a personalised reflection on the journal text
    const reflection = await callChatAPI({
      userText: `Please provide a brief, thoughtful AI reflection on this journal entry. Be warm, insightful, and personalised — not generic.\n\nJournal entry:\n${entry.text}`,
      messages: [],
      companionName: state.companion.name,
      userName: 'Seeker',
      userGoal: state.intention || 'personal growth',
      userLevel: state.level,
    })
    ...
    setState(prev => {
      const entries = prev.journalEntries.map(e =>
        e.id === entryId ? { ...e, aiReflection: reflection } : e
      )
      return { ...prev, journalEntries: entries, xp: prev.xp + 20 }
    })
  }
